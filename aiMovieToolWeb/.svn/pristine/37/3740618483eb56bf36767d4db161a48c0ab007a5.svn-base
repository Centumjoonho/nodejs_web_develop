...
$(document).ready(function(){
	...
	//공급사명 검색
	$("#sch_button_scompanyName").click(function() {
		var scompany_name = $("#sch_value_scompanyName").val();

		$.ajax({
			type : "POST",
			url : "<c:out value='/common/api/searchSupplycompany.json' />",
			data : {"scompany_name" : scompany_name},
			dataType : "json",
			async : "false",
			success : function(data) {
				...
				$(data).each(function() {
					...
					tags.append("<tr>");
					tags.append("<td><input type=\"radio\" name=\"scompany_id\" class=\"radio\" value=\"" +  this["scompany_id"] + "\"/></td>");
					tags.append("<input type=\"hidden\" name=\"" + this["scompany_id"] + "_scompany_name\" value=\"" +  this["scompany_name"] + "\"/>");
					tags.append("<td>" + this["scompany_name"] + "</td>");
					tags.append("<td>" + this["scompany_id"] + "</td>");
					tags.append("</tr>");

					disp_str = disp_str + tags.toString();
				});
				$("#disp_html_scompanyName").html(disp_str);
			},
	...
	//공급사명 선택
	$('#layer-pop_close_scompanyName').click(function() {
		var scompany_id = $("input:radio[name=scompany_id]:checked").val();
		var scompany_name = $("input:hidden[name=" + scompany_id+ "_scompany_name]").val();
		
		$("input[name='sch_scompany_name']").val(scompany_name);
	});
	...
	//정산배치실행
	$("#confirmBatch").click(function() {
		if($("#cal_start_date").val() == ""){
			alert("정산일을 선택해주세요.");
			$("#cal_start_date").focus();
			return false;
		}
		var paramList = new Array();
		paramList.push($("#cal_start_date").val());
		
		$.ajaxSettings.traditional = true;
		$.ajax({
			type : "POST",
			url : "<c:out value='/common/api/executeBatch.json' />",
			data : {"batch_type" : "calculation",
					"paramList" : paramList	},
			dataType : "json",
			async : "false",
			success : function(data) {
				if (data.resultCode == "1") {
					alert("성공");
					$("#layer-pop_close_batchLayer").click();
        		}else{
        			alert('<s:text name="common.msg.에러발생"/>');
        		}
			},
...
<body>
	<!-- Contents 시작 -->
	<div id="contents">
	    <h2>공급사 정산<span>Home > 정산 > 공급사 정산 </span></h2>
	    <div class="cboth"></div>
		...
		<div class="search">
			...
			<s:hidden id="cal_mst_id" name="cal_mst_id"/>
				...
				<table>
					...
					<tr>
						<th scope="row"><s:text name="common.lbl.공급사명"/></th>
						<td>
							<s:textfield name="sch_scompany_name" title="%{getText('common.lbl.공급사명')}" cssClass="text sizePer2" maxlength="128" maxbyte="128" />
							<a href="#scompanyName" class="btn-layer"><img src="<%=request.getContextPath() %>/resource/images/btn/btn_search.gif" alt="%{getText('common.btn.검색')}" /></a>
						</td>
		...
		<div class="listcasetit">
			...
			<div class="listcasebtn">
				<% if(loginInfo.getMember_id().equals("admin")){ %>
				<button id="#batchLayer" class="btnType04s btn-layer">정산실행</button>
				<% } %>
			...
			<!-- 공급사 발행당 정산 -->
			<table id="cal_scompany_S" class="board-list nohover" summary=".">
				<caption></caption>
				<colgroup>
				...
				<thead>
					<tr>
						...
						<th><s:text name="common.lbl.공급사명"/></th>
				...
				<tbody>
					<c:forEach items="${pageHelper.list}" var="list">
						<tr>
							...
							<td rowspan="3"><a href="#" onclick="fnDetail('${list.CAL_MST_ID}')">${list.SCOMPANY_NAME}</a></td>
			...
			<!-- 공급사 사용당 정산 -->
			<table id="cal_scompany_E" class="board-list nohover" summary="." style="display: none;">
				<caption></caption>
				<colgroup>
					<c:if test="${approvalChk == false}">
						<col style="width: 6%;" />
			...
			<!-- 공급사 퇴장 정산 -->
			<table id="cal_scompany_B" class="board-list nohover" summary="." style="display: none;">
				<caption></caption>
				<colgroup>
					<c:if test="${approvalChk == false}">
						<col style="width: 6%;" />
				...
				<tbody>
					<c:forEach items="${pageHelper.list}" var="list">
						<tr>
							...
							<td>${list.CAL_DT}</td>
							<td><a href="#" onclick="fnDetail('${list.CAL_MST_ID}')">${list.SCOMPANY_NAME}</a></td>
	...
	<!-- 공급사 조회 팝업 -->
	<div id="scompanyName" class="layer-pop">
		<p class="layer-tit"><img src="<%=request.getContextPath() %>/resource/images/common/popup_title.gif" /><s:text name="common.lbl.공급사조회"/></p>
		<div class="layer-cont">
			<div class="pop-search txetR">
				<s:textfield id="sch_value_scompanyName" type="text" class="text" title="%{getText('common.lbl.공급사입력')}" style="width: 385px;" onkeypress="enterKeyEvent(event,'sch_button_scompanyName');" />
				<button id="sch_button_scompanyName" class="btn-inquiryP" type="submit"><s:text name="common.btn.조회"/></button>
			</div>
			<div class="scroll-tbl" style="width: 450px; height: 135px;">
				<table class="board-list">
					<caption><s:text name="common.lbl.공급사명조회"/></caption>
					<colgroup>
						<col style="width: 5%" />
						<col />
						<col />
					...
					<thead>
						<tr>
							...
							<th scope="col"><s:text name="common.lbl.공급사명"/></th>
							<th scope="col"><s:text name="common.lbl.공급사ID"/></th>
						</tr>
					</thead>
					<tbody id="disp_html_scompanyName">
					</tbody>
				</table>
			</div>
			<div class="btn-center">
                <button id="layer-pop_close_scompanyName" class="btnType01 _pop_close"><s:text name="common.btn.확인"/></button>
			</div>
		</div>
		<div class="btn-close">
	...
	<!-- 정산 배치 실행 팝업 -->
	<div id="batchLayer" class="layer-pop">
		<p class="layer-tit"><img src="<%=request.getContextPath() %>/resource/images/common/popup_title.gif" />정산배치실행</p>
		<div class="layer-cont">
			<div class="pop-search txetc">
				정산일 : <span class="date"><s:textfield name="cal_start_date" cssClass="text datepicker sizePer1" /></span>
				<button id="confirmBatch" class="btnType01 ml20"><s:text name="common.btn.확인"/></button>
			</div>
		</div>
		<div class="btn-close">
			<button id="layer-pop_close_batchLayer" class="_pop_close">
				<img src="<%=request.getContextPath() %>/resource/images/btn/btn_close.gif" alt='<s:text name="common.btn.닫기"/>' />
			</button>
		</div>
	</div>
